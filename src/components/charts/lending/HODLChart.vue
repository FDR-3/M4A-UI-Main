...
<script setup lang="ts">
  ...
  import { HODLValueHistoryHashMap,
    HODL7DayProjectionHistoryHashMap,
    HODLUSDSHistoryHashMap,
    HODLUSDCHistoryHashMap,
    HODLSOLHistoryHashMap,
    HODLWEthHistoryHashMap,
    HODLWBtcHistoryHashMap } from './HODLHistory'
  ...
  function setChartData()
  {
    ...
    var allHODLTreasuryValues = []
    var allHODL7DayProjections = []
    ...
    const tokens =
    [
      { historyMap: HODLUSDSHistoryHashMap, tokenId: tokenIds.usdsTokenId, allData: [] as any[] },
      { historyMap: HODLUSDCHistoryHashMap, tokenId: tokenIds.usdcTokenId, allData: [] as any[] },
      { historyMap: HODLSOLHistoryHashMap,  tokenId: tokenIds.solTokenId,  allData: [] as any[] },
      { historyMap: HODLWEthHistoryHashMap, tokenId: tokenIds.wethTokenId, allData: [] as any[] },
      { historyMap: HODLWBtcHistoryHashMap, tokenId: tokenIds.wbtcTokenId, allData: [] as any[] }
    ]

    for(var year = startYear; year <= currentYear; year++)
    {
      var yearlyLabels: string[] = []
      var yearlyHODLTreasuryValues = []
      var yearlyHODL7DayProjections = []
      var yearlyHODLValueChartData = cloneDeep(valueBaseChartData)
      var yearlyHODLAmountChartData = getAmountBaseChart(gradientOffset)
      ...
      for(var month = 1; month <= maxMonth; month++)
      {
        ...
        const hodlTreasuryValue = HODLValueHistoryHashMap.get(month.toString() + '-' + year.toString())
        if(hodlTreasuryValue != undefined)
        {
          if(!labelAddedToAll)
          {
          ...
          allHODLTreasuryValues.push(hodlTreasuryValue)
          yearlyHODLTreasuryValues.push(hodlTreasuryValue)
        }
        else
        {
          if(year == currentYear && (month == currentMonth || month == currentMonth-1))
          {
            ...
            allHODLTreasuryValues.push(props.currentTreasuryValue)
            yearlyHODLTreasuryValues.push(props.currentTreasuryValue)
          }
          else
            yearlyHODLTreasuryValues.push(0)
        }

        const hodlProjectionMonthlyValue = HODL7DayProjectionHistoryHashMap.get(month.toString() + '-' + year.toString())
        if(hodlProjectionMonthlyValue != undefined)
        {
          allHODL7DayProjections.push(hodlProjectionMonthlyValue)
          yearlyHODL7DayProjections.push(hodlProjectionMonthlyValue)
        }
        else
        {
          if(year == currentYear && (month == currentMonth || month == currentMonth-1))
          {
            allHODL7DayProjections.push(props.current7DayProjection)
            yearlyHODL7DayProjections.push(props.current7DayProjection)
          }
          else
            yearlyHODL7DayProjections.push(0)
        }
      ...
      yearlyHODLValueChartData.labels = yearlyLabels
      yearlyHODLValueChartData.datasets[0].data = yearlyHODLTreasuryValues
      yearlyHODLValueChartData.datasets[1].data = yearlyHODL7DayProjections
      tempYearlyValueHashMap.set(year.toString(), yearlyHODLValueChartData)

      //Assign Amount data for the year
      yearlyHODLAmountChartData.labels = yearlyLabels
      tokens.forEach((_, i) =>
      {
        if(yearlyHODLAmountChartData.datasets[i])
          yearlyHODLAmountChartData.datasets[i].data = yearlyAmountDataLists[i]
      })
      tempYearlyAmountHashMap.set(year.toString(), yearlyHODLAmountChartData)
    }

    //Set Value "All" data
    tempAllValueChartData.labels = allLabels
    tempAllValueChartData.datasets[0].data = allHODLTreasuryValues
    tempAllValueChartData.datasets[1].data = allHODL7DayProjections
